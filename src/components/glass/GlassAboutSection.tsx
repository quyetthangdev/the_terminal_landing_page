...
export default function GlassAboutSection() {
  ...
  return (
    <section
      ...
      className={`relative py-32 bg-[#0d0d0d] overflow-hidden transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
    >
      {/* Background accent */}
      <div className="absolute top-0 left-0 w-96 h-96 bg-gold/5 rounded-full -translate-x-1/2 -translate-y-1/2 blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-white/[0.03] rounded-full translate-x-1/2 translate-y-1/2 blur-3xl pointer-events-none" />
      ...
      <div className="relative max-w-6xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="relative">
          ...
          {/* Glass label overlay */}
          <div className="absolute bottom-4 left-4 bg-black/40 backdrop-blur-md border border-white/15 px-4 py-3 rounded-sm">
            <p className="text-white/60 text-[10px] tracking-[0.3em]">THE TERMINAL</p>
            <p className="text-gold text-xs font-semibold tracking-wider">CAFE & EUROPEAN KITCHEN</p>
          </div>
        </div>

        {/* Glass content card */}
        <div className="bg-white/[0.05] backdrop-blur-sm border border-white/10 rounded-sm p-8 md:p-10">
          <p className="text-gold/70 text-[10px] tracking-[0.5em] mb-6">CÂU CHUYỆN</p>
          <h2 className="font-display text-3xl md:text-4xl font-bold text-white leading-tight mb-6">
            Điểm dừng chân<br />
            <em className="font-normal text-gold/80">giữa lòng thành phố</em>
          </h2>
